...
export async function fetchInfo(url) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const possibleCookies = [path.join(process.cwd(), 'cookies.txt'), path.join(DOWNLOADS_DIR, 'cookies.txt')];
    const cookiesFile = possibleCookies.find(fs.existsSync);
    if (cookiesFile) {
...
export async function startDownload(url, options, sendSSE) {
  ...
  try {
    ...
    const possibleCookies = [path.join(process.cwd(), 'cookies.txt'), path.join(DOWNLOADS_DIR, 'cookies.txt')];
    const cookiesFile = possibleCookies.find(fs.existsSync);
    if (cookiesFile && !url.includes("spotify.com")) {
      args.unshift("--cookies", cookiesFile);
    }
  } catch (e) {
